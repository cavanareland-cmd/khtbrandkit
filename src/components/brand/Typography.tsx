...
const scale = [
  ...
  { tag: "H3", size: "36px / 2.25rem", className: "text-xl md:text-4xl font-semibold", sample: "Paket Umrah Premium" },
  { tag: "H4", size: "24px / 1.5rem", className: "text-lg md:text-2xl font-semibold", sample: "Bersama KHT" },
  { tag: "H5", size: "20px / 1.25rem", className: "text-base md:text-xl font-medium", sample: "Manasik Terstruktur" },
];
...
const Typography = () => {
  ...
  return (
    <section id="typography" className="py-12 md:py-24 bg-gradient-soft relative">
      <div className="container px-4 mx-auto">
        <div className="max-w-2xl mb-8 flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="font-display text-3xl md:text-5xl font-bold text-secondary mb-4">Sistem Tipografi</h2>
            <p className="text-base md:text-lg text-muted-foreground">
              Pasangan font yang elegan: {displayName} untuk judul, {bodyName} untuk keterbacaan, dan {altName} untuk aksen.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16 mt-8">
          {fonts.map((f) => (
            <div key={f.name + f.className} className="rounded-2xl bg-card border border-border p-6 md:p-8 shadow-md hover:shadow-elegant transition-smooth group overflow-hidden">
              <div className={`${f.className} text-6xl md:text-8xl text-primary mb-6 leading-none transition-smooth group-hover:scale-105 origin-left`}>
                {f.sample}
              </div>
              <div className="space-y-2">
                <p className="font-alt text-[10px] uppercase tracking-widest text-accent">{f.role}</p>
                <h3 className={`${f.className} text-xl md:text-2xl font-semibold text-foreground`}>{f.name}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          ...
          <div className="rounded-2xl bg-card border border-border p-5 md:p-8 shadow-md">
            <h4 className="font-alt text-[10px] md:text-xs uppercase tracking-widest text-accent mb-6">Body Scale — {bodyName}</h4>
            <div className="space-y-6">
              {bodyScale.map((s) => (
                <div key={s.tag} className="border-b border-border/50 pb-4 last:border-0">
                  ...
                  <p className={`font-body ${s.className} text-foreground/80 leading-relaxed`}>
          ...
          <div className="relative space-y-6">
            <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
              <div>
                ...
                <h3 className="font-display text-2xl md:text-3xl font-bold">Uji Coba Font</h3>
                <p className="text-sm md:text-base text-secondary-foreground/70 mt-1">Ketik teks Anda untuk melihat tampilan font brand.</p>
              </div>
              <div className="flex flex-wrap gap-2">
                {(["display", "body", "alt"] as const).map((f) => (
                  <button
                    ...
                    className={`px-3 py-1.5 md:px-4 md:py-2 rounded-full text-[10px] md:text-xs font-alt uppercase tracking-widest transition-smooth ${
                      activeFont === f
            ...
            <input
              ...
              className="w-full bg-transparent border-b-2 border-secondary-glow/50 focus:border-accent outline-none py-3 font-body text-base md:text-lg text-secondary-foreground placeholder:text-secondary-foreground/40 transition-smooth"
            />

            <div className="bg-secondary-glow/20 rounded-2xl p-6 md:p-12 min-h-[150px] md:min-h-[200px] flex items-center justify-center">
              <p className={`font-${activeFont} text-2xl md:text-5xl text-center leading-tight break-words ${activeFont === "display" ? "italic" : ""}`}>
